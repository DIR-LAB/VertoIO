...
def calculate_score(prob, mppr, mrif, max_mppr, max_mrif):
    # Probability is already in the range of 0 to 1
    prob_score = prob
    ...
    # Combine scores - adjust weights if needed to prioritize one factor over another
    combined_score = prob_score + mppr_score + mrif_score

    return combined_score

def greedy_config(configs, probabilities, explor_config_list_all):
    ...
    if filtered_indices:
        ...
        for i, idx in enumerate(filtered_indices):
            ...
            scores.append(calculate_score(normalized_prob, explor_config_list_all[idx][0], explor_config_list_all[idx][1], max_mppr, max_mrif))
            print("[READ] Config: (", explor_config_list_all[idx][0], ', ',explor_config_list_all[idx][1], ') | Prob: ', probabilities[idx], ' | Score: ', scores[i])
...
def find_best_config(probabilities, explor_config_list_all):
    configs = list(range(len(explor_config_list_all)))

    chosen_config = greedy_config(configs, probabilities, explor_config_list_all)
...
def predict_vals(result_folder_path, wld_name, cur_osc_snap: osc_snapshot.OSC_Snapshot, client_osc_feats_df_dict, model, feats, explor_config_list_all, seq_length, obsvn_cnt, strtg_type):
    ...
    chosen_config = find_best_config(preds.tolist(), explor_config_list_all)
